...
class GpApiDigitalWalletTest extends TestCase
{
    ...
    public function testGooglePayEncrypted_LinkedRefund()
    {
        ...
        $refund = $transaction->refund()->withCurrency('EUR')->execute();
    ...
    public function testGooglePayEncrypted_Reverse()
    {
        ...
        $refund = $transaction->reverse()->withCurrency('EUR')->execute();

        $this->assertNotNull($refund);
        $this->assertEquals('SUCCESS', $refund->responseCode);
        $this->assertEquals(TransactionStatus::REVERSED, $refund->responseMessage);
    }
}