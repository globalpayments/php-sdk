...
use GlobalPayments\Api\Entities\Enums\{ChallengeRequestIndicator, ExemptStatus, YesNoEnum};
...
class HPPAuthenticationConfigurationTest extends TestCase
{
    ...
    {
        $this->config->billingAddressRequired = YesNoEnum::YES;
        $errors = $this->config->validate();
        $this->assertEmpty($errors);
    }
    ...
    public function testValidateWithInvalidBillingAddressRequired()
    {
        ...
        $errors = $this->config->validate();
        $this->assertContains('Invalid billing address required value', $errors);
    }
    ...
    public function testToArrayWithAllValues()
    {
        ...
        $expected = [
            ...
            'billing_address_required' => YesNoEnum::YES
        ];
