...
class HPPDisplayConfiguration
{
    ...
    public function validate(): array
    {
        ...
        if (empty($this->iframe_dimensions_domain)) {
            $errors[] = 'iframe_dimensions_domain is required';
        } elseif (!filter_var($this->iframe_dimensions_domain, FILTER_VALIDATE_DOMAIN, FILTER_FLAG_HOSTNAME)) {
            $errors[] = 'iframe_dimensions_domain must be a valid domain';
        }
        
        if (!empty($this->iframe_response_domain) && 
            !filter_var($this->iframe_response_domain, FILTER_VALIDATE_DOMAIN, FILTER_FLAG_HOSTNAME)) {
            $errors[] = 'iframe_response_domain must be a valid domain';
        }
