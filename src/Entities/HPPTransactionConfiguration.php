...
class HPPTransactionConfiguration
{
    ...
    public function toArray(): array
    {
        ...
        if (!empty($this->captureMode)) {
            $result['capture_mode'] = $this->captureMode;
        }
        
        if (!is_null($this->currencyConversionMode)) {
            $result['currency_conversion_mode'] = is_bool($this->currencyConversionMode) 
                ? StringUtils::boolToYesNo($this->currencyConversionMode) 
                : "NO";
        }
        
        if (!empty($this->allowedPaymentMethods)) {
            $result['allowed_payment_methods'] = $this->allowedPaymentMethods;
        }
        
        if (!empty($this->usageMode)) {
            $result['usage_mode'] = $this->usageMode;
        }
        
        if (!empty($this->usageLimit)) {
            $result['usage_limit'] = $this->usageLimit;
        }
