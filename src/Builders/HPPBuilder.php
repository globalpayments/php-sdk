...
class HPPBuilder extends AuthorizationBuilder
{
    ...
    */
     ...
     * @return HPPBuilder this
     */
    public function withAddressMatchIndicator(bool $indicator): self
    {
    ...
    public function withTransactionConfig(
        string $channel = 'CNP',
        string $country = 'GB',
        string $captureMode = CaptureMode::AUTO,
        array $allowedPaymentMethods = ['CARD'],
        string $usageMode = PaymentMethodUsageMode::SINGLE,
        string $usageLimit = '1'
    ): self
    {
        ...
        $this->transactionConfig->usageMode = $usageMode;
     ...
     * @return HPPBuilder this
     */
    public function withCurrencyConversionMode(bool $currencyConversionMode): self
    {
    ...
    public function withAuthentication(
        string $preference = ChallengeRequestIndicator::CHALLENGE_PREFERRED,
        string $exemptStatus = ExemptStatus::LOW_VALUE,
        bool $billingAddressRequired = true
    ): self
    {
        // Validate challenge preference
        try {
            ChallengeRequestIndicator::validate($preference);
        } catch (\Exception $e) {
            throw new ArgumentException('Invalid challenge preference');
        }
        
        // Validate exempt status
        try {
            ExemptStatus::validate($exemptStatus);
        } catch (\Exception $e) {
            throw new ArgumentException('Invalid exempt status');
        }
        
        // Validate billing address required (should be boolean)
        if (!is_bool($billingAddressRequired)) {
            throw new ArgumentException('Billing address required must be a boolean value');
        }
        
        $this->authConfig->preference = $preference;
        $this->authConfig->exemptStatus = $exemptStatus;
     ...
     * @return HPPBuilder this
     */
    public function withPaymentMethodConfig(string $storageMode = HPPStorageModes::PROMPT): self
    {
        $this->paymentMethodConfig->storageMode = $storageMode;
        return $this;
    }
     ...
     * 
     */
    public function withApm(bool $shippingAddressEnabled = true, bool $addressOverride = true): self
    {
     ...
     * @return HPPBuilder this
     */
    public function withShipping(bool $shippable = false, ?string $shippingAmount = null): self
    {
        if (!is_bool($shippable)) {
            throw new ArgumentException('Shippable must be a boolean value');
        }
        
        $this->HPPData->shippable = $shippable;
    ...
    {
        // Validate using reflection to check against the enum
        $reflection = new \ReflectionClass(HPPTypes::class);
        $validTypes = array_values($reflection->getConstants());
        
        if (!in_array($type, $validTypes)) {
            throw new ArgumentException('Invalid type value. Must be one of: ' . implode(', ', $validTypes));
        }
        $this->HPPData->type = $type;
        return $this;
    }
    ...
    {
        $reflection = new \ReflectionClass(HPPFunctions::class);
        $validFunctions = array_values($reflection->getConstants());
        
        if (!in_array($function, $validFunctions)) {
            throw new ArgumentException('Invalid function value. Must be one of: ' . implode(', ', $validFunctions));
        }
        $this->HPPData->function = $function;
        return $this;
    }
