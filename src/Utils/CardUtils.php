...
class CardUtils
{
     ...
     * @return Card
     */
    public static function generateCard($builder, $gatewayProvider, &$maskedValues)
    {
        ...
        if ($paymentMethod instanceof ITrackData) {
            ...
            if ($transactionType == TransactionType::SALE) {
                if (empty($card->track)) {
                    ...
                    $maskedValues = ProtectSensitiveData::hideValue('payment_method.card.number', $paymentMethod->pan, 4);
                    if (!empty($paymentMethod->expiry)) {
                        ...
                        $maskedValues = ProtectSensitiveData::hideValues(
                            [
            ...
            $maskedValues = ProtectSensitiveData::hideValue('payment_method.card.number', $paymentMethod->number, 4, 6);
            if (!empty($paymentMethod->expMonth)) {
                $card->expiry_month = str_pad($paymentMethod->expMonth, 2, '0', STR_PAD_LEFT);
                $maskedValues = ProtectSensitiveData::hideValue('payment_method.card.expiry_month', $card->expiry_month);
            }
            if (!empty($paymentMethod->expYear)) {
                $card->expiry_year = substr(str_pad($paymentMethod->expYear, 4, '0', STR_PAD_LEFT), 2, 2);
                $maskedValues = ProtectSensitiveData::hideValue('payment_method.card.expiry_year', $card->expiry_year);
            }
            if (!empty($paymentMethod->cvn)) {
                $card->cvv = $paymentMethod->cvn;
                $maskedValues = ProtectSensitiveData::hideValue('payment_method.card.cvv', $paymentMethod->cvn);
